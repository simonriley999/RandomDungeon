...
public class ObjectPooler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (Pool pool in pools)
        {
            ...
            // objHolder.name = pool.tag + "Holder";
            Queue<GameObject> objectPool = new Queue<GameObject>();
            for (int i=0;i<pool.size;i++)
            {
                // GameObject obj = Instantiate(pool.prefab,transform.position,Quaternion.identity,objHolder.transform);
    ...
    public GameObject SpawnFromPool(string tag,Vector3 postion,Quaternion rotation)
    {
        ...
        {
            Debug.LogWarning("Pool with tag " + tag + " is does not exsist");
            return null;
        }
